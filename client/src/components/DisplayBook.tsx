...
export const DisplayDbBook: ({ item, format }: { item: DbBookInfo; format: string }) => JSX.Element = ({ item, format }: { item: DbBookInfo; format: string }) => {
  ...
  const handleChange = (e:React.ChangeEvent<HTMLSelectElement>) => {
    ...
    navigate("/update", {state: { bookInfo: item, source: "db", selectedStatus: (e.target as HTMLSelectElement).value}})
  }
  ...
  return (
    ...
    <div className='bookInfo' key={item.id}>
      ...
      <select name="bookshelfSelect" id="bookshelfSelect" onChange={handleChange}>
        <option value="read" selected={item.status === "read" }>Read</option>
        <option value="toRead" selected={item.status === "toRead" }>Want to Read</option>
        <option value="currentlyReading" selected={item.status === "currentlyReading" }>Currently Reading</option>
      </select>
     {item.dateRead ?<h3>Date read: {item.dateRead?.toLocaleDateString("en-US")}</h3> :null}
